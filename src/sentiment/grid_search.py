...
# 2. Load & Filter Data
df = pd.read_csv("casse_labeled_dataset.csv")
...
def train_and_eval(lr, batch_size, weight_decay):
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        model_name, num_labels=len(allowed_labels)
    ).to(device)
...
for epoch in range(epochs):
    ...
    for batch in train_loader:
        ...
        outputs = model(inputs, mask, labels=labels)
        loss = outputs.loss
...
# # 9. Save Final Model
# save_dir = "fine_tuned_sentiment_bert"

# model.save_pretrained(save_dir)
# tokenizer.save_pretrained(save_dir)

# # Upload to Hugging Face Hub
# repo_name = "fine-tuned-sentiment-bert"  
