...
# Mark parent artifacts as having SATD if child has SATD
with open(JSONL_PATH, "r", encoding="utf-8") as f:
    for line in f:
        a = json.loads(line)
        if a.get("satd_label") not in (None, "non_debt"):
...
# Propagation analysis (strict continuous path)
chain_counts = Counter()
repo_chains = defaultdict(Counter)

for cid, comment in tqdm(comments.items(), desc="Analyzing propagation"):
    if not is_satd(comment):
        continue

    repo = comment["repo"]
    depth = 1  # comment itself
...
# Results
print("\nSATD PROPAGATION DEPTH (Strict Continuous Path)")
